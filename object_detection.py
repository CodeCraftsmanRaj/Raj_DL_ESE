...
for i, img in enumerate(sample_images):
    ...
    print(f"Image {i}: Detected {len(boxes)} objects, Avg Confidence: {detection_results[-1]['avg_confidence']:.4f}")
...
print(f"Total objects detected across {len(sample_images)} images: {len(multi_object_results)}")
...
degraded_images = [
    cv2.GaussianBlur(img, (5, 5), 0) for img in sample_images
]

degraded_results = []

for i, img in enumerate(degraded_images):
    results = model.predict(img, verbose=False)
    boxes = results[0].boxes
    confidences = boxes.conf.cpu().numpy() if hasattr(boxes.conf, 'cpu') else boxes.conf
...
plt.subplot(2, 3, 5)
plt.plot(img_ids, [detection_results[i]['avg_confidence'] for i in range(len(detection_results))], marker='o', label='Avg Confidence')
plt.xlabel('Image ID')
...
failure_analysis = {
    ...
    'false_positives': len([r for r in multi_object_results if r['confidence'] > 0.9]),
